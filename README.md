...
健康检查失败或反向隧道中断返回 `503`。
飞书客户端请求会先在公网入口执行免登，再把一次性授权码带到局域网页面并立即
从地址栏清除。这样飞书 JSAPI 始终运行在后台登记的公网 URL 上，而业务页面和
后续请求仍直接访问局域网后端。桥接页优先使用 `requestAccess`，失败或无响应时
回退到 `requestAuthCode`，并为接口调用设置超时。Gateway Agent 会把官方 H5 SDK
缓存在进程内并通过受访问控制的同源路径提供，避免飞书 WebView 直接连接外部 CDN。
普通浏览器继续使用直接 `302`。
