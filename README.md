...
健康检查失败或反向隧道中断返回 `503`。
飞书客户端请求会先在公网入口通过 OAuth 授权码重定向完成登录，再把一次性授权码
带到局域网页面并立即从地址栏清除；后续请求仍直接访问局域网后端。该流程不依赖
H5 JSAPI，一次性 `state` 只在 Agent 进程内短期保存。飞书后台 OAuth 重定向 URL
必须包含 `http://47.100.74.169/`，并发布
`contact:user.base:readonly` 权限。授权请求和后端换取用户令牌时使用完全相同的
公网回调地址。普通浏览器继续使用直接 `302`。
