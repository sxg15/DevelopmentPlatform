...
5. 飞书客户端被重定向到飞书 OAuth 授权页；授权结果通过一次性 `state` 回到
   公网入口，再携带授权码跳转到局域网。授权请求包含
   `contact:user.base:readonly`，整个流程不依赖 H5 JSAPI。
6. 非同一网络返回 `403`；升级、后端不可用或隧道断开返回 `503`。

`403` 页面会说明局域网、指定 VPN 和重新检测步骤；`503` 页面会显示维护或健康检查
状态并每 5 秒自动检测。状态页由 Agent 本地生成，不加载公网图片、脚本或样式，也
不会显示客户端 IP 和内部诊断数据。

飞书开发者后台的 OAuth 重定向 URL 必须包含
`http://47.100.74.169/`，并与 `publicEntry.baseUrl` 完全一致。应用还必须发布
`contact:user.base:readonly` 权限；后端仅在处理该公网 OAuth 流程产生的授权码时
把同一个回调地址传给令牌交换接口。
