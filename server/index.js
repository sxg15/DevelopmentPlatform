...
const clientDir = isProduction ? path.join(rootDir, 'client') : path.join(rootDir, 'Publish/client');
...
async function fetchCachedBitableRecords(token, tableConfig, cacheKeyPrefix, ttlMs) {
  const cacheKey = [
    'records',
    cacheKeyPrefix,
...
function getBitableFieldNames(tableConfig) {
...
function collectBitableFieldNames(value, names) {
  if (!value) {
    return;
  }
...
function formatFeishuApiError(message) {
...
async function fetchBitableTables(token, appToken) {
  const tables = [];
  let pageToken = '';
...
function fetchCachedBitableTables(token, appToken) {
...
function fetchCachedBitableFields(token, appToken, tableId) {
  return getCachedValue(
    bitableFieldsCache,
...
function invalidateBitableFieldsCache(appToken, tableId) {
  bitableFieldsCache.delete(getBitableFieldCacheKey(appToken, tableId));
}

function getBitableFieldCacheKey(appToken, tableId) {
  return `${appToken || ''}|${tableId || ''}`;
}

async function createBitableTextField(token, appToken, tableId, fieldName) {
...
async function ensureBitableTextField(token, appToken, tableId, fieldName) {
  const fields = await fetchCachedBitableFields(token, appToken, tableId);
...
async function ensureCachedBitableTextField(token, appToken, tableId, fieldName) {
  const fields = await fetchCachedBitableFields(token, appToken, tableId);
  const existingField = fields.find((field) => String(field.field_name || field.fieldName || '') === fieldName);
  if (existingField) {
    return fields;
...
async function getCachedValue(cache, key, ttlMs, loader) {
  const now = Date.now();
...
function getLocalUrls(currentPort) {
...
function validateProjectBaseConfig() {
...
function validateToolPermissionConfig() {
...
function blockDirectConfigAccess(request, response, next) {
